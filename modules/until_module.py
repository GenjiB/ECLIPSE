...
class CrossEn(nn.Module):
    ...
    def forward(self, sim_matrix, guide_dis=None):
...
class yb_CrossEn(nn.Module):
    def __init__(self,):
        super(yb_CrossEn, self).__init__()

    def forward(self, sim_matrix, id_dict, vid_id, opt=None):
        id_dict = id_dict.flatten()

        from ipdb import set_trace

        logpt = F.log_softmax(sim_matrix, dim=-1)
        sim_loss = []
        for my_idx in range(len(vid_id)):
            vid_loc = (vid_id[my_idx] == id_dict).nonzero(as_tuple=True)[0]
            sim_loss.append(-logpt[my_idx, vid_loc])

        return sum(sim_loss)/len(sim_loss)


        # gt_loc = []
        # for my_idx in range(len(vid_id)):
        #     vid_loc = (vid_id[my_idx] == id_dict).nonzero(as_tuple=True)[0]
        #     gt_loc.append(vid_loc.item())

        # while True:
        #     perm = torch.randperm(sim_matrix.size(1))
        #     rand_idx = perm[:opt.yb_sample_num]

        #     if not any(x in gt_loc for x in rand_idx.tolist()):
        #         break

        # new_sim_matrix = torch.cat((sim_matrix[:,gt_loc], sim_matrix[:,rand_idx]), dim=-1)
