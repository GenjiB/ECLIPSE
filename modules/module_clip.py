...
class ResidualAttentionBlock(nn.Module):
	...
	def av_attention(self, x: torch.Tensor, video_shape=None):
		...
		if self.opt.yb_dual:
			...
			### -------> att: column normalization
			if self.opt.yb_reverse_norm:
				num_tokens = x.shape[0]
				att_weight = self.vis_audio_attn(x_a, x, x_a.repeat(num_tokens,1,1), need_weights=True, attn_mask=attn_mask_)[1]
...
class VisualTransformer(nn.Module):
	def __init__(self, input_resolution: int, patch_size: int, width: int, layers: int, heads: int, output_dim: int,
		...
		self.myViT = timm.create_model('vit_base_patch32_224_in21k', pretrained=True)
...
class CLIP(nn.Module):
	def __init__(self,
		...
		## -----> rebuttal add:
		self.mlp_audio = nn.Linear(512,512)
		## <----
	...
	@property
	def dtype(self):
		# return self.visual.myViT.blocks[0].attn.qkv.weight.dtype
		# rebuttal
	...
	def encode_image(self, image, audio, return_hidden=False, video_frame=-1, video_shape=None, is_train=False):
		...
		hidden = self.visual(image.type(self.dtype), audio.type(self.dtype), video_frame=video_frame, video_shape=video_shape, is_train=is_train).type(self.dtype)
		# 
