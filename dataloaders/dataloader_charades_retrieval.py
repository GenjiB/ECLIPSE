...
class charades_DataLoader(Dataset):
	def __init__(
		...
		self.my_gt = {}
		self.all_list = glob.glob('/playpen-iop/yblin/Charades/Charades_v1_480/*.mp4')

		### ----------> buy caption
		# with open('/playpen-iop/yblin/Charades/charades_sta_'+self.subset+'.txt') as f:
		# 	lines = f.readlines()

		# for line in lines:
		# 	name = line.split(' ')[0]
		# 	if name not in self.my_gt:
		# 		self.my_gt[name] = []
		# 		self.my_gt[name].append(line.replace(name, '').replace('\n', ''))
		# 	else:
		# 		self.my_gt[name].append(line.replace(name, '').replace('\n', ''))
		#### <-----------


		df = pandas.read_csv('/playpen-iop/yblin/Charades/Charades_v1_'+self.subset+'.csv')
		
		for idx in range(len(df)):
			name = df['id'][idx]
			if name not in self.my_gt:
				self.my_gt[name] = df['script'][idx]
	...
	def _get_text(self, name, sub_id):
		...
		for i in range(k):
			...
			# 	if content.split(' ')[1] not in words_dict:
			# 		words_dict[content.split(' ')[1]] = content.split('##')[-1]
			# 	else:
			# 		words_dict[content.split(' ')[1]] = words_dict[content.split(' ')[1]] +' '+content.split('##')[-1]

			
			# my_words = ""
			# for key in sorted(words_dict):
			my_words = self.my_gt[name]
	...
	def _get_rawvideo_yb(self, name, s, e):
		...
		video_folder ='/playpen-iop/yblin/Charades/raw_frames/'+ name
		## sort video order ----->
		# time_interval = {}
		# frame_list = []
		# for content in self.my_gt[name]:
		...
		frame_list = glob.glob(video_folder+'/*.jpg')
	...
	def _get_audio(self, name, s, e):
		...
		audio_folder = '/playpen-iop/yblin/Charades/raw_audio'
		...
		video_folder ='/playpen-iop/yblin/Charades/VGGSound_Audio_features_10s_aligned/'+ name
		# ## sort video order ----->
		# time_interval = {}
		# frame_list = []
		# for content in self.my_gt[name]:
		...
		frame_list = glob.glob(video_folder+'/*.pt')
		total_num_frames = len(frame_list)
		...
		if True:
			...
			for tmp_idx in sample_indx:
				...
				fbank = torch.load(frame_list[tmp_idx], map_location=torch.device('cpu')).double()
