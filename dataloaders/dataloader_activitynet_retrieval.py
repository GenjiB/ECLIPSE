...
class ActivityNet_DataLoader(Dataset):
	def __init__(
		...
		for root, dub_dir, video_files in os.walk(self.features_path):
			for video_file in video_files:
				...
				### ------> rebuttal added 
				# if video_id_ in VGGSound_train_vid and self.subset == 'val': # rebuttal added
				# 	overlap_count += 1
		...
		## add path for v1-3
		for root, dub_dir, video_files in os.walk(self.features_path.replace('v1-2','v1-3')):
			for video_file in video_files:
				video_id_ = ".".join(video_file.split(".")[:-1])
				video_id_ = video_id_[2:] # yb modify
				...
				### ------> rebuttal added 
				# if video_id_ in VGGSound_train_vid and self.subset == 'val':
				# 	overlap_count += 1
				# 	print('overlap:', overlap_count)
				# 	continue
				# file_path_ = os.path.join(root, video_file)
	...
	def _get_rawvideo_yb(self, idx, s, e):
		...
		if self.max_frames < (total_num_frames/3) or True:
			...
			for tmp_idx in sample_indx:
				# try:
				# tmp_img = torchvision.io.read_image(video_folder+'/v_'+idx+'/'+ str("{:04d}".format(tmp_idx))+ '.jpg').double()/255
	...
	def __getitem__(self, feature_idx):
		...
		pairs_text, pairs_mask, pairs_segment, starts, ends, raw_words = self._get_text(pseudo_video_id, sub_id)
		# video, video_mask = self._get_rawvideo(self.video_id_list[idx], starts, ends)

		# if self.subset == 'train':
			# video, video_mask = self._get_rawvideo_fixed_length(self.video_id_list[idx], starts, ends)
		video,_, video_mask = self._get_rawvideo_yb(self.video_id_list[idx], starts, ends)

		audio, audio_mask = self._get_audio(self.video_id_list[idx], starts, ends)
		...
		# mypath = glob.glob('/playpen-iop/yblin/v1-2/Activity_Videos/'+pseudo_video_id+'.*')
		
		# shutil.copyfile(mypath[0], '/playpen-iop/yblin/act_val_video/'+pseudo_video_id+'.'+mypath[0].split('.')[-1] )

		if self.subset=='train':
			text_pt = torch.load('/playpen-storage/yblin/v1-2/train_features_b32_f96/5341_text.pt')
			vis_pt = torch.load('/playpen-storage/yblin/v1-2/train_features_b32_f96/5341_vis.pt')
			return pairs_text, pairs_mask, pairs_segment, video, video_mask, audio, audio_mask, text_pt, vis_pt, idx
			
		else:
			my_data = {'path': self.save_path, 'words':raw_words}
			return pairs_text, pairs_mask, pairs_segment, video, video_mask, audio, audio_mask #,self.save_path #,my_data
			# return pairs_text, pairs_mask, pairs_segment, video, video_ori, video_mask, audio, audio_mask#, self.save_path