...
# from dataloaders.dataloader_activitynet_retrieval_demo import ActivityNet_DataLoader
...
from dataloaders.dataloader_youcook_short_retrieval import youcook_short_DataLoader
from dataloaders.dataloader_qvhighlight_retrieval import qvhighlight_DataLoader
...
def dataloader_youcook_test(args, tokenizer, subset="test"):
...
def dataloader_youcook_short_train(args, tokenizer):
    youcook_dataset = youcook_short_DataLoader(
        subset="train",
        data_path=args.data_path,
        features_path=args.features_path,
        max_words=args.max_words,
...
def dataloader_youcook_short_test(args, tokenizer, subset="test"):
    youcook_testset = youcook_short_DataLoader(
        subset=subset,
        data_path=args.data_path,
        features_path=args.features_path,
        max_words=args.max_words,
...
def dataloader_qvhighlight_train(args, tokenizer):
    qvhighlight_dataset = qvhighlight_DataLoader(
        subset="train",
        data_path=args.data_path,
        features_path=args.features_path,
        max_words=args.max_words,
    ...
    train_sampler = torch.utils.data.distributed.DistributedSampler(qvhighlight_dataset)
    dataloader = DataLoader(
        qvhighlight_dataset,
        batch_size=args.batch_size // args.n_gpu,
        num_workers=args.num_thread_reader,
    ...
    return dataloader, len(qvhighlight_dataset), train_sampler

def dataloader_qvhighlight_test(args, tokenizer, subset="test"):
    qvhighlight_testset = qvhighlight_DataLoader(
        subset=subset,
        data_path=args.data_path,
        features_path=args.features_path,
        max_words=args.max_words,
    ...
    dataloader_qvhighlight = DataLoader(
        qvhighlight_testset,
        batch_size=args.batch_size_val,
        num_workers=args.num_thread_reader,
        shuffle=False,
    ...
    return dataloader_qvhighlight, len(qvhighlight_testset)
...
DATALOADER_DICT["youcook"] = {"train":dataloader_youcook_train, "val":dataloader_youcook_test, "test":None}

DATALOADER_DICT["youcook_short"] = {"train":dataloader_youcook_short_train, "val":dataloader_youcook_short_test, "test":None}


DATALOADER_DICT["qvhighlight"] = {"train":dataloader_qvhighlight_train, "val":dataloader_qvhighlight_test, "test":dataloader_qvhighlight_test}
