...
class youcook_DataLoader(Dataset):
	def __init__(
		...
		self.all_list = glob.glob('/playpen-storage/yblin/yk2/raw_videos_all/low_all_'+self.subset+'/*.mp4')

		# -----> Rebuttal: checking overlap 457
		overlap_count = 0
		VGGSound_train_vid = []
		...
		for file_path in self.all_list:
			if file_path.split('/')[-1][:-4] in VGGSound_train_vid:
					overlap_count += 1
					print('overlap:', overlap_count)
		
		#  <---------

		f = open('/playpen-storage/yblin/yk2/raw_videos_all/youcookii_annotations_trainval.json')
		self.annotations = json.load(f)
		# self.annotations['database']["GLd3aX16zBg"]
		f.close()
	...
	def _get_text(self, pseudo_video_id, sub_id):
		...
		vid_name = self.all_list[pseudo_video_id].split('/')[-1][:-4]
		# caption = self.pseudo_caption_dict[pseudo_video_id]
		k = 1
		r_ind = [sub_id]
		...
		for i in range(k):
			...
			start_, end_ = 0, self.annotations['database'][vid_name]['annotations'][-1]['segment'][-1]
			
			my_words = ""
			for cat_idx in range(len(self.annotations['database'][vid_name]['annotations'])):
				my_words = my_words + self.annotations['database'][vid_name]['annotations'][cat_idx]['sentence'] + '. '
	...
	def _get_rawvideo_yb(self, idx, s, e):
		...
		video_folder = '/playpen-storage/yblin/yk2/raw_videos_all/videos_frame_' + self.subset +'/' +vid_name
		# video_folder = video_folder.replace('playpen-storage','playpen-storage')
	...
	def _get_audio(self, idx, s, e):
		...
		vid_name = self.all_list[idx].split('/')[-1][:-4]
		audio_folder = '/playpen-storage/yblin/yk2/audio_raw_' + self.subset
	...
	def __getitem__(self, idx):
		...
		video, video_mask = self._get_rawvideo_yb(idx, starts, ends)
