...
def get_args(description='CLIP4Clip on Retrieval Task'):
	...
	parser.add_argument("--yb_reverse_norm", default=0, type=int, help="change normalization column")
...
def load_model(epoch, args, n_gpu, device, model_file=None):
	if model_file is None or len(model_file) == 0:
		...
		model_file = '/playpen/yblin/CLIP4Clip/ckpts/ckpt_activity_retrieval_looseType/pytorch_model.b32_f96_43.0'
	...
	with torch.no_grad():
		...
		for bid, batch in enumerate(test_dataloader):
			...
			if multi_sentence_:
				...
				## yb: add mean
				visual_output = visual_output.mean(dim=1).unsqueeze(1)
...
def main():
	...
	if hasattr(model, "clip") and args.freeze_layer_num > -1:
		for name, param in model.clip.named_parameters():
			...
			if name.find("ln_final.") == 0 or name.find("text_projection") == 0 or name.find("logit_scale") == 0 \
					...
					or ('conv_audio' in name) or ('positional_audio' in name) or ('audio_cls' in name) or ('resnet' in name) or ('myViT' in name):  # or
				# print(" grad:", name)
	...
	if args.do_train:
		...
		vis_dict = torch.load('/playpen-storage/yblin/v1-2/b32-dict-f96/vis_dict.pt')
		id_dict = torch.load('/playpen-storage/yblin/v1-2/b32-dict-f96/id_dict.pt')
		text_dict = torch.load('/playpen-storage/yblin/v1-2/b32-dict-f96/text_dict.pt')
		...
		for epoch in range(args.epochs):
			...
			if args.local_rank == 0:
				...
				if stop_count == early_stop:
					# early stop
					# os.system("kill $(ps aux | grep yb_factor_aduio=" +str(args.yb_factor_aduio) +"| grep -v grep | awk '{print $2}') ")
